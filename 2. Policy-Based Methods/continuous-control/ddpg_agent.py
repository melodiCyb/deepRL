...
EPSILON_DECAY = 1e-6     # decay for epsilon in noise
PERIOD = 20              # learning timestep interval
...
class Agent():
    ...
    def __init__(self, state_size, action_size, random_seed=0):
        ...
        # Actor Network
        self.actor_local = Actor(state_size, action_size, random_seed, leakiness=0.01).to(device)
        self.actor_target = Actor(state_size, action_size, random_seed, leakiness=0.01).to(device)
        self.actor_optimizer = optim.Adam(self.actor_local.parameters(), lr=LR_ACTOR)
        
        # Critic Network    
        self.critic_local = Critic(state_size, action_size, random_seed, leakiness=0.01).to(device)
        self.critic_target = Critic(state_size, action_size, random_seed, leakiness=0.01).to(device)
    ...
    def learn_from_memory(self, timestep):
        if len(self.memory) > BATCH_SIZE and timestep % PERIOD == 0:
